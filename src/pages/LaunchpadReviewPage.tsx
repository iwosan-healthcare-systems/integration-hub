...
function StatusEditor({ submission, onSaved }: { submission: IdeaSubmission; onSaved: () => Promise<void> }) {
  ...
  const stages: IdeaStatus[] = ['submitted', 'under_review', 'successful', 'under_implementation', 'concluded_pilot'];
  const canSelect = (next: IdeaStatus) => isAdmin || next === submission.status || (!['concluded_pilot', 'rejected'].includes(submission.status) && (next === 'rejected' || stages.indexOf(next) > stages.indexOf(submission.status)));
  const locked = !isAdmin && (submission.status === 'concluded_pilot' || submission.status === 'rejected');
  ...
  return <div className="space-y-1.5"><div className="flex gap-2"><select aria-label={`Status for ${submission.reference}`} className={`${selectClass} min-w-36`} value={status} onChange={e=>{ const next = e.target.value as IdeaStatus; if (canSelect(next)) setStatus(next); }} disabled={busy || locked}>{Object.entries(LAUNCHPAD_STATUSES).map(([key,label])=><option key={key} value={key} disabled={!canSelect(key as IdeaStatus)}>{label}</option>)}</select><Button size="sm" className="h-8" disabled={busy || reasonMissing || !canSelect(status) || status===submission.status} onClick={save}>{busy?'Saving...':'Save'}</Button></div>{status === 'rejected' && submission.status !== 'rejected' && <div className="space-y-1"><Label htmlFor={`rejection-${submission.id}`}>Reason for parking *</Label><Textarea id={`rejection-${submission.id}`} value={comment} onChange={e=>setComment(e.target.value)} maxLength={2000} rows={3} disabled={busy} placeholder="Explain why this idea was parked. The submitter will see this comment." /></div>}{!isAdmin && submission.status !== 'submitted' && <p className="text-xs text-muted-foreground">{locked ? 'Only an admin can change a final decision.' : 'Only an admin can revert a status.'}</p>}</div>;
}
export default function LaunchpadReviewPage({ panel = false }: { panel?: boolean }) {
  ...
  return <LaunchpadShell panel={panel}><Seo title="Review dashboard | LaunchPad" path={panel ? "/admin/launchpad" : "/launchpad/review"} description="Track and review LaunchPad ideas." /><header className="flex flex-wrap gap-4 items-start justify-between"><div><h1 className="font-display text-3xl font-bold">Review dashboard</h1><p className="text-muted-foreground mt-2">Ideas from across Iwosan, in one place.</p></div><div className="flex gap-2"><Button variant="outline" aria-label="Refresh submissions" onClick={()=>refresh()} disabled={result.isFetching}><RefreshCw className={`h-4 w-4 ${result.isFetching?'animate-spin':''}`}/></Button><Button onClick={download} disabled={exporting||result.isPending||!!result.error}><Download className="h-4 w-4 mr-2"/>{exporting?'Exporting…':'Export CSV'}</Button></div></header>
    ...
    {result.data && <>
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-2" aria-label="Filter submissions by status">
        {[['','All submissions'],...Object.entries(LAUNCHPAD_STATUSES)].map(([key,label]) => <button type="button" key={key} aria-pressed={filters.status===key} aria-controls="review-submissions" onClick={()=>selectStatus(key)} className={`rounded-xl border p-3 text-left transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${key ? (filters.status===key ? launchpadStatusColors[key as IdeaStatus].active : launchpadStatusColors[key as IdeaStatus].card) : filters.status===key ? 'border-accent bg-accent/10 ring-1 ring-accent/30' : 'bg-card hover:border-accent/50 hover:bg-accent/5'}`}><span className="block text-[11px] text-muted-foreground">{label}</span><span className={`block text-2xl font-bold mt-1 ${key ? launchpadStatusColors[key as IdeaStatus].text : ''}`}>{key ? result.data.statusSummary[key as IdeaStatus] : Object.values(result.data.statusSummary).reduce((sum,count)=>sum+count,0)}</span></button>)}
      </div>
