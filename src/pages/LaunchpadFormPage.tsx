import { parseLaunchpadFunding } from '@/lib/launchpadFunding';
...
const formSections = [
  ...
  { title: 'Implementing Your Idea', icon: FlaskConical, hint: 'Implement your ideas through a project, with clear measures of success' },
...
function Section({ number, title, children }: { number: number; title: string; children: React.ReactNode }) {
  ...
  return <fieldset id={`idea-section-${number}`} className="lp-form-section rounded-3xl border bg-card p-5 sm:p-7 space-y-5 shadow-sm">
    ...
    <p className="flex items-center gap-2 text-xs text-accent pb-4 border-b"><Icon className="h-4 w-4 shrink-0 text-accent" />{hint}</p>
    {children}
  </fieldset>;
}
...
export default function LaunchpadFormPage() {
  ...
  const [values,setValues] = useState<string[]>([]); const [busy,setBusy] = useState(false); const [error,setError] = useState('');
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    let funding: number;
    try { funding = parseLaunchpadFunding(text('funding')); }
    catch (e) { setError(e instanceof Error ? e.message : 'Enter a valid amount.'); requestAnimationFrame(()=>errorRef.current?.focus()); return; }
    const answers: IdeaAnswers = { department:text('department'), managerName:text('managerName'), managerEmail:text('managerEmail'), problem:text('problem'), idea:text('idea'), implementationPlan:text('implementationPlan'), duration:text('duration'), measurement:text('measurement'), risks:text('risks'), owner:text('owner'), values, funding, startDate:text('startDate'), managerSupported:data.get('managerSupported')==='yes', mdApproved:data.get('mdApproved')==='yes' };
  ...
  return <LaunchpadShell><Seo path="/launchpad/your_idea" title="Submit an idea | LaunchPad" description="Share your idea for a better Iwosan." /><div className="max-w-3xl mx-auto space-y-6">
    ...
    <LaunchpadReveal><aside className="overflow-hidden rounded-3xl border border-accent/25 shadow-sm">
      ...
      <div className="bg-card p-6 sm:p-7 space-y-5"><div className="grid sm:grid-cols-2 gap-5"><div><p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-2">The problem</p><p className="text-sm font-medium">Patients miss appointments.</p></div><div><p className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-accent mb-2"><MessageCircle className="h-3.5 w-3.5" /> The idea</p><p className="text-sm font-medium">SMS/WhatsApp appointment reminders.</p></div></div>
        <dl className="grid grid-cols-2 gap-3"><div className="rounded-2xl bg-muted/50 p-4"><CalendarDays className="h-4 w-4 text-accent mb-2" /><dt className="text-xs text-muted-foreground">Project duration</dt><dd className="text-lg font-bold mt-1">8 weeks</dd></div><div className="rounded-2xl bg-muted/50 p-4"><Wallet className="h-4 w-4 text-accent mb-2" /><dt className="text-xs text-muted-foreground">Project budget</dt><dd className="text-lg font-bold mt-1">₦95,000</dd></div></dl>
    ...
    <form onSubmit={submit} className="space-y-7">
      ...
      <Section number={4} title="Implementing Your Idea">
        <Field name="implementationPlan" label="How will you implement this idea?" long maxLength={5000}/>
        <Field name="measurement" label="How will you know it worked - what KPIs/outcomes are you testing for" long maxLength={5000}/>
        <Field name="startDate" label="When would you like to start working on idea?" type="date"/>
        <Field name="duration" label="How much time do you think it will take to achieve this objective?" maxLength={500} placeholder="e.g. 8 weeks"/>
        <div><Field name="funding" label="How much funding will you need? (The standard funding limit is N100,000 per idea)" type="text" inputMode="decimal" pattern="[0-9]+([.][0-9]{1,2})?" maxLength={12} placeholder="e.g. 20000 or 20000.50" title="Enter a non-negative amount with at most two decimal places" aria-describedby="funding-help"/><p id="funding-help" className="text-xs text-muted-foreground mt-2">Higher requests will be flagged for reviewers.</p></div>
        <Field name="risks" label="What could prevent your project/solution from working?" long required={false} maxLength={5000}/>
      </Section>
      <Section number={5} title="Ownership and approval"><Field name="owner" label="Who will be responsible for delivering the project?" defaultValue={user?.name ?? ''}/><p className="text-xs text-muted-foreground">Your name is prefilled. Change it if someone else will deliver the project.</p><fieldset className="space-y-3"><legend className="text-sm font-medium mb-2">Has your line manager seen and supported this idea? <span className="text-destructive">*</span></legend><div className="flex gap-6">{['yes','no'].map(value=><label key={value} className="flex items-center gap-2 rounded-xl border px-5 py-3 text-sm cursor-pointer transition-colors hover:border-accent/50 has-[:checked]:bg-accent/10 has-[:checked]:border-accent/50"><input className="h-4 w-4 accent-current" type="radio" name="managerSupported" value={value} required/>{value==='yes'?'Yes':'No'}</label>)}</div></fieldset><fieldset className="space-y-3"><legend className="text-sm font-medium mb-2">Has your MD approved this idea? <span className="text-destructive">*</span></legend><div className="flex gap-6">{['yes','no'].map(value=><label key={value} className="flex items-center gap-2 rounded-xl border px-5 py-3 text-sm cursor-pointer transition-colors hover:border-accent/50 has-[:checked]:bg-accent/10 has-[:checked]:border-accent/50"><input className="h-4 w-4 accent-current" type="radio" name="mdApproved" value={value} required/>{value==='yes'?'Yes':'No'}</label>)}</div></fieldset></Section>
