...
export default function LaunchpadFormPage() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const answers: IdeaAnswers = { department:text('department'), managerName:text('managerName'), managerEmail:text('managerEmail'), problem:text('problem'), idea:text('idea'), testMethod:text('testMethod'), testTeam:text('testTeam'), measurement:text('measurement'), risks:text('risks'), owner:text('owner'), values, funding, startDate:text('startDate'), endDate:text('endDate'), managerSupported:data.get('managerSupported')==='yes' };
  ...
  return <LaunchpadShell><Seo path="/launchpad/submit" title="Submit an idea | LaunchPad" description="Share your idea for a better Iwosan." /><div className="max-w-3xl mx-auto space-y-6">
    ...
    <form onSubmit={submit} className="space-y-7">
      ...
      <Section number={4} title="The pilot"><Field name="testMethod" label="How will you test out this idea?" long maxLength={5000}/><Field name="testTeam" label="Who will you be working with to test out this idea?" long maxLength={5000}/><div><Field name="funding" label="How much funding do you need? (₦)" type="text" inputMode="decimal" pattern="[0-9]+([.][0-9]{1,2})?" maxLength={12} placeholder="e.g. 20000 or 20000.50" title="Enter a non-negative amount with at most two decimal places" aria-describedby="funding-help"/><p id="funding-help" className="text-sm text-accent mt-2 font-medium">The standard funding limit is ₦100,000 per idea. Higher requests will be flagged for reviewers.</p></div><div className="space-y-2"><p className="text-sm font-medium">When will the pilot start and end?</p><div className="grid sm:grid-cols-2 gap-4"><Field name="startDate" label="Pilot start" type="date" value={startDate} onChange={e=>setStartDate(e.target.value)}/><Field name="endDate" label="Pilot end" type="date" min={startDate || undefined}/></div></div><Field name="measurement" label="How will you know it worked? Tell us what you will measure and the result you are aiming for." maxLength={500}/><Field name="risks" label="What could prevent the pilot from working?" long required={false} maxLength={5000}/></Section>
