...
const values = [
  ...
  { name: 'Ethical', icon: ShieldCheck, text: 'Greater integrity, safety and responsibility.', example: 'Small habits. A cleaner ward.', detail: 'Implement recycling and waste segregation with your ward team.', impact: 'Sustainability & compliance', amount: '₦60,000', color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300', tint: 'from-emerald-100/70 dark:from-emerald-950/30' },
...
const steps = [
  ...
  ['Submit', 'Describe the problem, the process you will improve, your project and the funding you need.'],
  ['MD/CEO approval', 'Your MD/CEO reviews, approves and funds the idea from your entity’s budget.'],
  ['Guidance', 'Before funds are released, the PAC advises, challenges assumptions and logs the idea.'],
  ['You deliver', 'The idea owner leads the project, measures results and accounts for the funds.'],
  ['Scale', 'Retain or retire the project, then package proven ideas for other facilities to adopt.'],
];
  ...
  return <LaunchpadShell>
    ...
    <section className="lp-enter relative isolate overflow-hidden rounded-[2rem] bg-primary text-primary-foreground p-7 sm:p-12">
      ...
      <div className="relative flex flex-col lg:flex-row lg:items-center gap-8"><div className="max-w-2xl space-y-6">
        ...
        <p className="text-base sm:text-lg text-white/85 leading-relaxed">LaunchPad gives Iwosan Team members the support to turn practical ideas into better care. Develop your idea, secure backing and lead delivery, with funding from your facility and guidance from the Launchpad Peer Advisory Committee (PAC).</p>
    ...
    <section aria-labelledby="values-heading" className="space-y-5 py-3">
      <LaunchpadReveal><p className="text-accent text-xs font-bold uppercase tracking-widest">Start with a value</p><h2 id="values-heading" className="text-2xl font-bold mt-2">Start with the values</h2></LaunchpadReveal>
    ...
    <div className="grid md:grid-cols-2 gap-5 py-3">
      <LaunchpadReveal className="h-full"><section className="h-full rounded-3xl border p-7 space-y-4"><Users className="h-7 w-7 text-accent" /><h2 className="text-xl font-bold">Guidance from across Iwosan</h2><p className="text-sm text-muted-foreground leading-relaxed">An eight-member committee of peers, bringing together four Group colleagues and four colleagues from across our Iwosan sites, will support idea owners and help strengthen promising ideas.</p><p className="text-sm text-muted-foreground">Please note: your entity’s MD/CEO remains the approving authority. The Committee will only support ideas and projects that have been approved by site MDs/CEOs.</p></section></LaunchpadReveal>
      <LaunchpadReveal delay={100} className="h-full"><section className="h-full rounded-3xl border border-accent/20 bg-gradient-to-br from-accent/10 to-transparent p-7 space-y-4"><div className="flex items-center gap-2 text-sm font-medium"><Wallet className="h-5 w-5 text-accent" /> Funding per idea</div><p className="text-sm text-muted-foreground leading-relaxed">Ideas may be funded up to a maximum of NGN100,000 per idea. Sites may fund up to four ideas per cycle.</p></section></LaunchpadReveal>
    </div>
    ...
    <section aria-labelledby="ideas-heading" className="rounded-[2rem] border border-accent/10 bg-gradient-to-b from-accent/5 via-muted/30 to-transparent p-5 sm:p-8 space-y-7">
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {values.map((v, i) => <LaunchpadReveal key={v.name} delay={(i % 3) * 80} className="h-full"><article className={`lp-card relative h-full overflow-hidden rounded-2xl border border-border/70 bg-gradient-to-br ${v.tint} to-card p-4 flex flex-col`}>
          <div className="flex items-start justify-between gap-3 mb-3"><span className={`lp-card-icon rounded-xl p-2 ${v.color}`}><v.icon className="h-5 w-5" /></span><span className="text-[10px] uppercase tracking-widest text-muted-foreground pt-2">Example idea {i + 1}</span></div>
      ...
      <p className="text-xs text-muted-foreground leading-relaxed">Amounts are illustrative. Sites may fund up to four ideas per cycle; you can submit more than one idea.</p>
    </section>
